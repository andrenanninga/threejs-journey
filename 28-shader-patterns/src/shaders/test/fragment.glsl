...
void main()
{
	...
	// // Pattern 22
	// float strength = floor(vUv.x * 10.0) / 20.0 + floor(vUv.y * 10.0) / 20.0;
	...
	// // Pattern 35
	// float strength = step(0.01, abs(distance(vUv, vec2(0.5)) - 0.35));
	...
	// vec2 waveUv = vec2(
	// 	vUv.x,
	// 	vUv.y + sin(vUv.x * 15.0) * 0.1
	// );
	// float strength = 1.0 - step(0.01, abs(distance(waveUv, vec2(0.5)) - 0.35));
	...
	// float radius = 0.25 + sinusoid / 50.0;
	...
	// // Pattern 48
	// float strength = sin(1.0 - abs(cnoise(vUv * 10.0)) * 20.0);
